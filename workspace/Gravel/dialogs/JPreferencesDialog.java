 ...
 */public class JPreferencesDialog extends JDialog implements ActionListener, ItemListener, ChangeListener, CaretListener, KeyListener
{
	...
	private void formtogp()
	{
		...
		gp.setIntValue("edge.arrow_alpha",Math.round(chEdge.getArrow().getAngle()));
		gp.setFloatValue("edge.arrow_part",chEdge.getArrow().getPart());
		gp.setIntValue("edge.arrow_size", Math.round(chEdge.getArrow().getSize()));
		gp.setFloatValue("edge.arrow_pos", chEdge.getArrow().getPos());
