 ...
 */public class JPreferencesDialog extends JDialog implements ActionListener, ItemListener, ChangeListener, CaretListener
{
	...
	private void formtogp()
	{
		...
		gp.setIntValue("edge.arralpha",Math.round(chEdge.getArrowAngle()));
		gp.setIntValue("edge.arrpart",Math.round(chEdge.getArrowPart()*100));
		gp.setIntValue("edge.arrsize", Math.round(chEdge.getArrowSize()));
		gp.setIntValue("edge.arrpos", Math.round(chEdge.getArrowPos()*100));
