...
public class GraphAction {
	...
	private void exchangeSubgraphMembership(int ItemType, int itemindex, VGraph first, VGraph second) throws GraphActionException
	{
		...
		while (si.hasNext())
		{
			...
			else if (ItemType==GraphConstraints.EDGE)
			{
	...
	private VGraph doReplace(VGraph graph) throws GraphActionException
	{
		...
		switch(Objecttype)
		{
			...
			case GraphConstraints.SUBGRAPH:
				...
				graph.pushNotify(new GraphMessage(GraphConstraints.SUBGRAPH,newSubgraph.getIndex(),GraphConstraints.UPDATE|GraphConstraints.BLOCK_START,GraphConstraints.GRAPH_ALL_ELEMENTS));
	...
	public VGraph redoAction(VGraph graph) throws GraphActionException
	{
		...
		if ((Objecttype==GraphConstraints.GRAPH)||(Objecttype==GraphConstraints.SELECTION)) //Move Selection from old graph to new one
